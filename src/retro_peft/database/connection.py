...
class DatabaseManager:
    ...
    def _initialize_sqlite(self):
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_training_runs_adapter ON training_runs(adapter_config_id)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_performance_metrics_run ON performance_metrics(training_run_id)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_model_versions_adapter ON model_versions(adapter_config_id)"
            )
    ...
    def get_connection(self):
        ...
        if self.config.db_type == "sqlite":
            conn = sqlite3.connect(self.config.db_path, timeout=30.0, check_same_thread=False)
    ...
    def create_adapter_config(
        self, name: str, adapter_type: str, base_model: str, config: Dict[str, Any]
    ) -> int:
    ...
    def create_training_run(
        self, adapter_config_id: int, run_name: str, hyperparameters: Dict[str, Any]
    ) -> int:
