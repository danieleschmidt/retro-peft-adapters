...
class InMemoryRateLimiter:
    ...
    def _create_global_limiter(self):
        ...
        if self.config.algorithm == RateLimitAlgorithm.TOKEN_BUCKET:
            self.global_limiter = TokenBucket(
                rate=self.config.global_limit, capacity=int(self.config.global_limit * 2)
            )
    ...
    def _get_or_create_limiter(self, key: str, endpoint: str, user_id: str) -> Any:
        ...
        if self.config.algorithm == RateLimitAlgorithm.TOKEN_BUCKET:
            ...
            limiter = SlidingWindowCounter(limit=int(rate * window), window_size=window)
        elif self.config.algorithm == RateLimitAlgorithm.FIXED_WINDOW:
            limiter = FixedWindowCounter(limit=int(rate * window), window_size=window)
        else:
    ...
    async def check_rate_limit(
        self, key: str, endpoint: str, user_id: Optional[str] = None
    ) -> RateLimitResult:
        """
...
class RedisRateLimiter:
    ...
    async def check_rate_limit(
        ...
        try:
            ...
            if self.config.algorithm == RateLimitAlgorithm.TOKEN_BUCKET:
                result = self._token_bucket_script(keys=[redis_key], args=[burst, rate, 1, now])
                ...
                result = self._sliding_window_script(keys=[redis_key], args=[window, limit, now])
            ...
            return RateLimitResult(
                allowed=True, remaining=1000, reset_time=time.time() + 60, limit_type="fallback"
            )

    async def _fallback_rate_limit(
        self, key: str, endpoint: str, user_id: Optional[str]
    ) -> RateLimitResult:
