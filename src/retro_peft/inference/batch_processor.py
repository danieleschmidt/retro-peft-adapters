...
class BatchProcessor:
    ...
    def process_batch(
        ...
        remaining_requests = [req for req in batch_requests if req.id not in completed_ids]
        ...
        print(
            f"Processing {len(remaining_requests)} requests "
        ...
        if self.max_workers > 1:
            # Parallel processing
            results.extend(self._process_parallel(remaining_requests, return_retrieval_info))
        else:
            # Sequential processing
            results.extend(self._process_sequential(remaining_requests, return_retrieval_info))
    ...
    def _process_sequential(
        self, requests: List[BatchRequest], return_retrieval_info: bool
    ) -> List[BatchResult]:
    ...
    def _process_parallel(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Submit all batches
            future_to_batch = {
                executor.submit(self._process_single_batch, batch, return_retrieval_info): batch_idx
    ...
    def _process_single_batch(
        self, batch: List[BatchRequest], return_retrieval_info: bool
    ) -> List[BatchResult]:
        ...
        for request in batch:
            ...
            try:
                ...
                result = BatchResult(
                    ...
                    retrieval_info=(
                        response.get("retrieval_info") if return_retrieval_info else None
                    ),
    ...
    def process_file(
        ...
        **kwargs,
        ...
        requests = self._load_requests_from_file(input_file, input_format, prompt_field, id_field)
    ...
    def _load_requests_from_file(
        self, input_file: str, input_format: str, prompt_field: str, id_field: str
    ) -> List[BatchRequest]:
    ...
    def _save_results_to_file(
        self, results: List[BatchResult], output_file: str, output_format: str
    ):
