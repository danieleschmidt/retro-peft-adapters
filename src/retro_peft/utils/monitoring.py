    ...
    def _cleanup_old_metrics(self):
        ...
        for metric_name in list(self._metrics.keys()):
            ...
            if not self._metrics[metric_name]:
                ...
                if cleaned_count > 0:
                    self.logger.debug(
                        f"Cleaned {cleaned_count} old metrics for {metric_name}"
                    )
        ...
        def decorator(func):
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Function {func_name} completed in {duration_ms:.3f}ms"
                    )
                    ...
                    self.logger.error(
                        f"Function {func_name} failed after {duration_ms:.3f}ms: {e}"
                    )
                    raise
...
class MemoryMonitorContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self.monitor.metrics_collector.record_gauge(
            f"memory.{self.operation}.delta_mb",
            memory_delta,
            self.tags
        )
        
        self.monitor.metrics_collector.record_gauge(
            f"memory.{self.operation}.peak_mb",
            end_memory,
            self.tags
        )
    ...
    def run_checks(self) -> Dict[str, HealthCheck]:
        ...
        for name, check_func in self.checks.items():
            try:
                ...
                self.logger.debug(
                    f"Health check {name}: {result.status} - {result.message}"
                )
