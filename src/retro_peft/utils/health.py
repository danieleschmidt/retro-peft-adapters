        ...
        for directory in directories_to_check:
            if os.path.exists(directory):
                ...
                if usage_percent > 95 or free_gb < 1:
                    critical_issues.append(f"{directory}: {usage_percent:.1f}% used, {free_gb:.1f}GB free")
...
def check_network_connectivity() -> HealthCheck:
    ...
    try:
        ...
        connectivity_tests = [
            ...
            ("HTTP Connectivity", "http://httpbin.org/status/200", 
             lambda: urllib.request.urlopen("http://httpbin.org/status/200", timeout=5)),
            ("HTTPS Connectivity", "https://httpbin.org/status/200",
        ...
        "system_info": {
            ...
            "environment_variables": {
                ...
                if any(keyword in key.upper() for keyword in [
                    'CUDA', 'PATH', 'PYTHON', 'HOME', 'USER', 'LANG'
                ])
            }
        },
        "process_info": {
