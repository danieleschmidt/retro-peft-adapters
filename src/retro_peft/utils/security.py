    ...
    def validate_config_dict(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in config.items():
            ...
            if isinstance(value, str):
                ...
                raise ValueError(
                    f"Unsupported configuration value type for key {key}: {type(value)}"
                )
...
class SecurityManager:
            ...
            if self.config["enable_audit_logging"]:
                self._audit_log(
                    "input_validation_failed",
                    {
                        ...
                        "error": str(e),
                        "value_preview": (
                            str(value)[:100] if isinstance(value, str) else str(type(value))
                        ),
                    },
                )
            raise
    ...
    def scan_for_threats(self, text: str) -> Dict[str, Any]:
        ...
        """
        threats = {"detected": False, "threats": [], "risk_level": "low", "details": {}}
        ...
        if found_keywords:
            threats["threats"].append({"type": "suspicious_keywords", "keywords": found_keywords})
