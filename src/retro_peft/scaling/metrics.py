...
class ScalingMetrics:
    ...
    def record_request(
        ...
        self.metrics_collector.record_timer(
            f"service.{service}.latency",
            latency_ms,
            tags=labels
        )
        
        self.metrics_collector.record_counter(
            f"service.{service}.requests_total",
            1,
            tags={**(labels or {}), "success": str(success)}
        )
        ...
        with self._lock:
            ...
            self._service_metrics[service]["throughput_tracker"].record_request_end(error=not success)
    ...
    def get_scaling_recommendations(self) -> List[Dict[str, Any]]:
        ...
        if (throughput_stats["requests_per_second"] < 1.0 and 
            "cpu" in resource_stats and 
            resource_stats["cpu"]["avg"] < 20):
            recommendations.append({
                "type": "scale_down",
