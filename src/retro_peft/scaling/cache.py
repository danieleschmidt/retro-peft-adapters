...
class DiskCache:
    ...
    def __init__(
        self, cache_dir: str, max_size_mb: int = 1000, cleanup_interval: int = 3600  # 1 hour
    ):
        """
            ...
            try:
                ...
                with sqlite3.connect(self.db_path) as conn:
                    conn.execute(
                        ...
                        VALUES (?, ?, ?, ?, ?, ?)
                    """,
                        (key, filename, time.time(), ttl, file_size, json.dumps({})),
                    )
...
class VectorCache:
    ...
    def __init__(
        self, max_vectors: int = 100000, vector_dim: int = 768, similarity_threshold: float = 0.9
    ):
        """
    ...
    def get_similar(
        ...
        with self._lock:
            ...
            for idx in top_indices:
                results.append(
                    (self.keys[idx], self.metadata[idx].copy(), float(similarities[idx]))
                )
    ...
    def put(self, key: str, vector: np.ndarray, metadata: Dict[str, Any]) -> None:
        """
...
class MultiLevelCache:
    ...
    def put_vector(self, key: str, vector: np.ndarray, metadata: Dict[str, Any]) -> None:
