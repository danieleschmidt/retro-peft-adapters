...
class RequestRouter:
    ...
    def select_backend(
        ...
        with self._lock:
            ...
            healthy_backends = [
                b for b in backends 
                if b.status == BackendStatus.HEALTHY and 
            ...
            if not healthy_backends:
                ...
                healthy_backends = [
                    b for b in backends 
                    if b.status == BackendStatus.DEGRADED and 
                       b.active_connections < b.max_connections
                ]
...
class LoadBalancer:
    ...
    def __init__(
        ...
        self.circuit_breakers = {
            b.id: CircuitBreaker(cb_config) for b in backends
        }
    ...
    def route_request(
        ...
        with self._lock:
            ...
            # Select backend
            selected_backend = self.router.select_backend(
                available_backends, 
                request_context
            )
    ...
    def record_request_result(
        ...
        with self._lock:
            ...
            if success:
                ...
                if error:
                    self.logger.warning(
                        f"Request to backend {backend.id} failed: {error}"
                    )
            ...
            for backend_id, backend in self.backends.items():
                ...
                status[backend_id] = {
                    ...
                    "success_rate": (
                        ...
                        if backend.total_requests > 0 else 0.0
                    ),
