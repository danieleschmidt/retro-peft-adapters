...
class DynamicGatedAdapterFusion(nn.Module):
    ...
    def forward(
        ...
        stacked_outputs = torch.stack(adapter_outputs, dim=-1)  # [batch_size, seq_len, output_dim, num_adapters]
        
        # Apply gating weights
        gate_weights_expanded = gate_weights.unsqueeze(1).unsqueeze(2)  # [batch_size, 1, 1, num_adapters]
...
class CrossAttentionAdapterFusion(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        concatenated = torch.cat(adapter_outputs, dim=1)  # [batch_size, seq_len * num_adapters, output_dim]
        ...
        # Final fusion
        concatenated_final = torch.cat(aggregated_outputs, dim=-1)  # [batch_size, seq_len, output_dim * num_adapters]
