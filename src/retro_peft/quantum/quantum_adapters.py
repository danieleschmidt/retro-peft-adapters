...
class HybridQuantumClassicalAdapter(nn.Module):
    ...
    def __init__(
        ...
        self.advantage_detector = QuantumAdvantageDetector(
            input_dim=input_dim,
            threshold=config.quantum_advantage_threshold
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        classical_weight = (routing_probs[:, 1] * (1.0 - quantum_advantage_score)).unsqueeze(-1).unsqueeze(-1)
    ...
    def get_usage_statistics(self) -> Dict[str, Any]:
        ...
        if self.performance_history:
            avg_quantum_advantage = np.mean([h["quantum_advantage_score"] for h in self.performance_history])
...
class VariationalQuantumCircuit(nn.Module):
    ...
    def _rx_matrix(self, angle: torch.Tensor) -> torch.Tensor:
        ...
        return torch.stack([
            torch.stack([cos_val, -1j * sin_val]),
            torch.stack([-1j * sin_val, cos_val])
        ])
...
def demonstrate_quantum_adapters():
    ...
    try:
        ...
        with torch.no_grad():
            ...
            print(f"✓ Output statistics: mean={quantum_output.mean():.4f}, std={quantum_output.std():.4f}")
        ...
        with torch.no_grad():
            ...
            print(f"✓ Output statistics: mean={hybrid_output.mean():.4f}, std={hybrid_output.std():.4f}")
