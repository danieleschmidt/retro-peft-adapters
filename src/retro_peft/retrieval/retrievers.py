...
class BaseRetriever(ABC):
    ...
    @abstractmethod
    def search(
        self, query_embedding: np.ndarray, k: int = 5, filter_dict: Optional[Dict[str, Any]] = None
...
class QdrantRetriever(BaseRetriever):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            from qdrant_client.models import Distance, FieldCondition, Filter, Range, VectorParams
    ...
    def add_documents(self, embeddings: np.ndarray, metadata: List[Dict[str, Any]]) -> None:
        ...
        for i, (embedding, meta) in enumerate(zip(embeddings, metadata)):
            point = PointStruct(
                ...
                payload={"text": meta.get("text", ""), "metadata": meta.get("metadata", {})},
            )
            points.append(point)
    ...
    def search(
        ...
        if filter_dict:
            conditions = []
            for key, value in filter_dict.items():
                condition = self.FieldCondition(key=f"metadata.{key}", match={"value": value})
...
class HybridRetriever(BaseRetriever):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            embedding_dim=dense_retriever.embedding_dim, encoder=dense_retriever.encoder, **kwargs
        )
    ...
    def search(
        ...
        if self.sparse_retriever is not None:
            ...
            results, scores = self._fuse_results(
                dense_results, dense_scores, sparse_results, sparse_scores
            )
    ...
    def _weighted_score_fusion(
        ...
        for result, score in zip(dense_results, norm_dense_scores):
            text_key = result.get("text", "")
            result_map[text_key] = {"result": result, "dense_score": score, "sparse_score": 0.0}
        ...
        for result, score in zip(sparse_results, norm_sparse_scores):
            ...
            if text_key in result_map:
                ...
                result_map[text_key] = {"result": result, "dense_score": 0.0, "sparse_score": score}

        # Compute weighted scores
        fused_results = []
        for data in result_map.values():
            weighted_score = (
                self.dense_weight * data["dense_score"] + self.sparse_weight * data["sparse_score"]
            )
