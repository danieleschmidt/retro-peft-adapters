...
class MultiTaskDataset(Dataset):
    ...
    def _compute_task_weights(self) -> Dict[str, float]:
        ...
        if self.task_sampling == "uniform":
            ...
            return {task: len(data) / total_size for task, data in self.task_datasets.items()}
...
class MultiTaskRetroTrainer:
        ...
        for task_name, task_config in self.tasks.items():
            if task_name.endswith("_classification"):
                ...
                self.task_heads[task_name] = nn.Linear(
                    self.model.base_model.config.hidden_size, num_classes
                )
            elif task_name.endswith("_retrieval"):
    ...
    def train(
        ...
        optimizer = torch.optim.AdamW(
            list(self.model.parameters()) + list(self.task_heads.parameters()), lr=learning_rate
        )
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                if (batch_idx + 1) % self.gradient_accumulation_steps == 0:
                    ...
                    torch.nn.utils.clip_grad_norm_(
                        list(self.model.parameters()) + list(self.task_heads.parameters()), 1.0
                    )
    ...
    def _compute_retrieval_loss(self, task_batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        query_embeddings = self._encode_texts(
            task_batch["query_input_ids"], task_batch["query_attention_mask"]
        )

        doc_embeddings = self._encode_texts(
            task_batch["doc_input_ids"], task_batch["doc_attention_mask"]
        )
    ...
    def _compute_classification_loss(
        self, task_batch: Dict[str, torch.Tensor], task_name: str
    ) -> torch.Tensor:
        """Compute classification loss"""
        # Get embeddings
        embeddings = self._encode_texts(task_batch["input_ids"], task_batch["attention_mask"])
    ...
    def _evaluate_retrieval(self, samples: List[Dict[str, Any]]) -> float:
        ...
        for sample in samples[:10]:
            ...
            doc_emb = self._encode_texts(doc_tokens["input_ids"], doc_tokens["attention_mask"])
    ...
    def _evaluate_classification(self, samples: List[Dict[str, Any]], task_name: str) -> float:
        ...
        for sample in samples[:10]:
            ...
            tokens = tokenizer(text, return_tensors="pt", padding=True, truncation=True).to(
                self.device
            )
    ...
    def _save_checkpoint(self, checkpoint_path: str, global_step: int):
        ...
        # Save task heads
        torch.save(self.task_heads.state_dict(), os.path.join(checkpoint_path, "task_heads.pt"))
