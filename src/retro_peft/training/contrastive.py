...
class ContrastiveRetrievalDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        negative_docs = self._sample_negative_documents(query_idx, positive_doc_idx)
        ...
        return {
            ...
            "negative_input_ids": torch.stack(
                [neg_tokens["input_ids"].squeeze(0) for neg_tokens in negative_tokens_list]
            ),
            "negative_attention_mask": torch.stack(
                [neg_tokens["attention_mask"].squeeze(0) for neg_tokens in negative_tokens_list]
            ),
            "query_idx": torch.tensor(query_idx),
            "positive_doc_idx": torch.tensor(positive_doc_idx),
        }

    def _sample_negative_documents(self, query_idx: int, positive_doc_idx: int) -> List[str]:
        ...
        if (
            ...
            and num_hard_negatives > 0
            and positive_doc_idx < len(self.document_embeddings)
        ):
        ...
        if remaining_negatives > 0:
            ...
            random_negatives = random.sample(
                available_indices, min(remaining_negatives, len(available_indices))
            )
    ...
    def _sample_hard_negatives(
        self, positive_doc_idx: int, num_hard_negatives: int, exclude_indices: set
    ) -> List[int]:
        ...
        for i, doc_embedding in enumerate(self.document_embeddings):
            if i not in exclude_indices and i != positive_doc_idx:
                similarity = F.cosine_similarity(
                    positive_embedding.unsqueeze(0), doc_embedding.unsqueeze(0)
                ).item()
        ...
        selected_negatives = random.sample(
            hard_negative_pool, min(num_hard_negatives, len(hard_negative_pool))
        )
...
class ContrastiveRetrievalTrainer:
    ...
    def train(
        ...
        scheduler = self._get_linear_schedule_with_warmup(optimizer, warmup_steps, total_steps)
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                if dynamic_hard_negatives and hard_negative_mining and global_step % 1000 == 0:
    ...
    def _compute_contrastive_loss(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        negative_input_ids = batch["negative_input_ids"].view(
            -1, batch["negative_input_ids"].size(-1)
        )
        negative_attention_mask = batch["negative_attention_mask"].view(
        ...
        if self.loss_type == "contrastive":
            ...
            loss = self._triplet_loss(query_embeddings, positive_embeddings, negative_embeddings)
        elif self.loss_type == "circle":
            loss = self._circle_loss(query_embeddings, positive_embeddings, negative_embeddings)
        else:
            raise ValueError(f"Unknown loss type: {self.loss_type}")

        return {"total_loss": loss, "contrastive_loss": loss}
    ...
    def _contrastive_loss(
        ...
        positive_similarities = (
            F.cosine_similarity(query_embeddings, positive_embeddings, dim=1) / self.temperature
        )

        # Negative similarities
        negative_similarities = (
            torch.bmm(query_embeddings.unsqueeze(1), negative_embeddings.transpose(1, 2)).squeeze(1)
            / self.temperature
        )
        ...
        if self.in_batch_negatives:
            ...
            in_batch_similarities = torch.mm(query_embeddings, all_positives.t()) / self.temperature
            ...
            all_negative_similarities = torch.cat(
                [negative_similarities, in_batch_similarities], dim=1
            )
        else:
            all_negative_similarities = negative_similarities

        # Contrastive loss
        logits = torch.cat([positive_similarities.unsqueeze(1), all_negative_similarities], dim=1)
    ...
    def _triplet_loss(
        ...
        # Positive distances
        positive_distances = 1 - F.cosine_similarity(query_embeddings, positive_embeddings, dim=1)

        # Negative distances (take minimum for hardest negative)
        negative_similarities = torch.bmm(
            query_embeddings.unsqueeze(1), negative_embeddings.transpose(1, 2)
        ).squeeze(1)
    ...
    def _circle_loss(
        ...
        """Compute circle loss"""
        # Positive similarities
        positive_similarities = F.cosine_similarity(query_embeddings, positive_embeddings, dim=1)
        ...
        loss = torch.log(
            1
            + torch.sum(torch.exp(self.gamma * negative_term), dim=1)
    ...
    def _compute_document_embeddings(self, dataset: ContrastiveRetrievalDataset) -> torch.Tensor:
        ...
        with torch.no_grad():
            for i in range(0, len(dataset.documents), batch_size):
                ...
                # Encode
                embeddings = self._encode_texts(tokens["input_ids"], tokens["attention_mask"])
    ...
    def _evaluate(self, dataset: ContrastiveRetrievalDataset) -> Dict[str, float]:
        ...
        return {"avg_positive_similarity": total_similarity / num_samples}
    ...
    def _get_linear_schedule_with_warmup(
        self, optimizer, num_warmup_steps: int, num_training_steps: int
    ):
